...
describe('DSH browser artifact', () => {
  ...
  it('keeps host actions on one row and nests Agent setup under the selected backend', () => {
    ...
    expect(js).toContain('确认部署')
    expect(js).toContain('点击部署后会在这台主机上执行官方安装命令。')
    expect(js).toContain('浏览中…')
    expect(js).toContain('归档中…')
    expect(js).toContain('停止中…')
    expect(js).toContain('用户主动停止')
    expect(js).toContain('提交中…')
    expect(js).toContain('正在接入实时通道')
    expect(js).toContain('会话通道中断')
    expect(js).toContain('实时通道断开，正在自动重连')
    expect(js).toContain('本轮失败')
    expect(js).toContain('待确认')
    ...
    expect(js).toContain('重新连接')
    expect(js).toContain('在当前会话重开')
  })
})
